...
12. **MiniMax 429 misclassified as malformed/no_change (2026-07-18 ~02:52 incident)** — account hit rate limit/quota; every call returned HTTP 429. The invoker retries 4× then raises, and `detect_quota_wall` exists on that path — but the PORT lane classifies the resulting empty response as `malformed_response` (3 retry attempts × ~45s each, a1/a2/a3 artifacts with `tokens: 0 in / 0 out`) and the GLOBALS lane stamps benign `no_change`, so workers marched on polluting run history instead of quota-pausing. One pass logged 10/12 malformed. Fix: in both lanes, detect the zero-token/emptiness + 429 signature and route to the existing `_yield_for_quota_pause` machinery; also re-queue the ~10 functions falsely marked malformed at 03:03–03:15 and audit `no_change` globals runs from 03:09–03:15.
