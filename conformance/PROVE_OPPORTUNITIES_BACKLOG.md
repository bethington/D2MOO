...
4. **[POLICY SHIPPED 2026-07-19] Review/promotion backlog** — the pile had grown 67→116. "Review" is now defined (see SHIPPING_PROMOTION_PLAN.md §"Review policy v1" + `tools/promote_pending_review.py`): auto-promote only on evidence beyond the drafting model's own word (battletested / V1-vetted / discriminating-synth ≥10 vec), queue the rest for V1 adversarial re-proof. First run: **15 promoted** to `proven_live`, **78 written to `v1_reproof_queue.json`** (plain-live 30 + weak 28 + low-vec 18 + delegate 2), 13 already re-prove-queued, 10 static-lane proofs need a **ledger backfill** (the static lane never appended registry rows — small residual fix). Also fixed: `proven_live` was missing from `_PORT_TERMINAL_STATUSES` (promoted functions would have been re-selected by the port worker). REMAINING: run the V1 loop over the 78 next oracle session; backfill the 10 static rows.
...
12. **[DONE 2026-07-19] HTTP worker-launch endpoint** — implemented during the MiniMax-429 downtime window: `POST /api/worker/start` {mode, provider, count, model, binary, continuous} (mode "globals" routes the globals path; ValueError → 409 with the lock/validation message), `POST /api/worker/stop` {worker_id}, `GET /api/worker/status` (roster snapshot, poll instead of holding a socket). Validation paths live-tested; first real launch pending MiniMax recovery. Socket.io handlers unchanged (UI unaffected).
