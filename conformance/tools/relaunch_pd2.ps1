# fix_relaunch_pd2.ps1 -- kill the wrongly-launched vanilla D2, then launch the
# REAL Project Diablo 2 conformance game (C:\Diablo2\ProjectD2\Game.exe) via the
# build-1.13c detours launcher with DIABLO2_PATCH=build-1.13c\patch (fresh D2Common,
# 21 globals) and the D2Debugger MCP bridge (:8790). Self-elevates (one UAC).
...
# ---- ELEVATED from here ----
Write-Host '[1/3] stopping the wrongly-launched game + launcher...'
...
Write-Host '[2/3] launching Project Diablo 2 (conformance build, debugger on :8790)...'
