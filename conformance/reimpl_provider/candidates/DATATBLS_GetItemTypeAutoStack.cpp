...
// D2MOO_REIMPL_EXPORT: DATATBLS_GetItemTypeAutoStack
// [abi_static] GLOBAL-TABLE getter (no model): g_pDataTables->records[idx*0xe4] + 0x13.
extern "C" uint32_t __stdcall DATATBLS_GetItemTypeAutoStack(int idx)
{
    if (idx < 0) return 0x0;
    ...
    return (uint32_t)*(uint8_t*)(rec + 0x13);
}
