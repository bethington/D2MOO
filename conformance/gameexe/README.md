...
| **`sub esp,N` differs** (the prologue immediate) | a **local buffer is the wrong size**. MSVC honours the declared array, so growing it (`szCommand[24]`→`[48]`) grows the frame and byte-matches the prologue. **Verify by disasm, not the scoreboard**: `short -N` is a *total-length* delta and can persist after the frame is fixed — a byte-exact prologue with a length gap means the residual is elsewhere (next row). | size the buffer to the original's `sub esp,N` |
| same CALLs, same memory ops, same control flow, but **different register assignment or a spill the original doesn't have** (return value in a callee-saved reg vs spilled to `[esp+N]`; direct `[eax+esi]` vs stack-relative `[esp+edi+N]`) | **register-allocation noise, not a source bug** — the optimiser's global scheduling, generally NOT reachable by restructuring C | **STOP.** Record the residual, drop to the behavioural (`CONF_TRACE`) tier. Grinding this burns budget for nothing — it is the hard tail the ladder exists for. |
