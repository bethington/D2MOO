...
void __fastcall PLAYERLIST_FreePlayerLists(Game* pGame, UnitAny* pPlayer)
{
    ...
    PlayerList* pPlayerList = pPlayerData->pPlayerList;
    while (pPlayerList)
    {
        PlayerList* pNext = pPlayerList->pNext;
...
{
    UnitAny* pPlayerArg = (UnitAny*)pArg;
    ...
    PlayerList* pPrevious = nullptr;
    for (PlayerList* pPlayerList = pPlayerData->pPlayerList; pPlayerList; pPlayerList = pPlayerList->pNext)
    {
...
{
    PlayerData* pPlayerData = UNITS_GetPlayerData(pPlayer1);
    if (pPlayer1 == pPlayer2 || STATES_CheckState(pPlayer2, STATE_PLAYERBODY) || STATES_CheckState(pPlayer1, STATE_PLAYERBODY))
    {
        return;
    }

    PlayerList* pPlayerList = PLAYERLIST_GetPlayerListRecordFromUnitGUID(pPlayer1, pPlayer2 ? pPlayer2->dwUnitId : -1);
    if (pPlayerList)
    {
        return;
    }

    pPlayerList = D2_CALLOC_STRC_POOL(pGame->pMemoryPool, PlayerList);
...
void __fastcall sub_6FCBAB50(Game* pGame, UnitAny* pPlayer, void* pArg)
{
    ...
    for (PlayerList* i = UNITS_GetPlayerData(pPlayer)->pPlayerList; i; i = i->pNext)
    {
...
void __fastcall PLAYERLIST_sub_6FCBAD60(Game* pGame, UnitAny* pPlayer)
{
    ...
    for (PlayerList* pPlayerList = pPlayerData->pPlayerList; pPlayerList; pPlayerList = pPlayerList->pNext)
    {
        ...
        {
            UnitAny* pOtherPlayer = SUNIT_GetServerUnit(pGame, UNIT_PLAYER, pPlayerList->nUnitGUID);
            if (pOtherPlayer)
            {
