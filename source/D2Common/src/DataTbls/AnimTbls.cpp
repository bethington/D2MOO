...
{
	AnimDataTable* pAnimDataTable = D2_CALLOC_STRC_POOL(nullptr, AnimDataTable);
...
;
	AnimDataBucket* pBucketBinaryData = (AnimDataBucket*)pAnimDataTable->pBinaryData;
	for (int i = 0; i < 256; ++i)
	{
		pAnimDataTable->pHashTableBucket[i] = pBucketBinaryData;
		const size_t nBucketSize = sizeof(pBucketBinaryData->nbEntries) + sizeof(AnimDataRecord) * pBucketBinaryData->nbEntries;
		pBucketBinaryData = (AnimDataBucket*)(((char*)pBucketBinaryData) + nBucketSize);
	}
...
static AnimDataRecord* DATATBLS_GetAnimDataRecordFromPath(char* szPath)
{
	ToUpperCase(szPath);
	const uint8_t nHash = SimpleStringHash(szPath);

	AnimDataTable* pAnimData = DATATBLS_GetAnimData();

	AnimDataBucket* pAnimDataBucket = pAnimData->pHashTableBucket[nHash];
	for (int i = 0; i < pAnimDataBucket->nbEntries; i++)
	{
		AnimDataRecord& rBucketEntry = pAnimDataBucket->aEntries[i];
...
const AnimDataRecord* __fastcall DATATBLS_GetAnimDataRecord(UnitAny* pUnit, int nClassId, int nMode, int nUnitType, Inventory* pInventory)
{
	...
	if (AnimDataRecord* pFound = DATATBLS_GetAnimDataRecordFromPath(szPath))
	{
		return pFound;
	}
	else
...
void __stdcall DATATBLS_UnitAnimInfoDebugSet(UnitAny* pUnit, int nAnimSpeed)
{
	int nWeaponClassCode = 0;
	char szPath[8] = {};
	D2Common_10885_COMPOSIT_unk(pUnit, szPath, &nWeaponClassCode, 0, 1, pUnit->pInventory, -1);
	if (AnimDataRecord* pRecord = DATATBLS_GetAnimDataRecordFromPath(szPath))
	{
...
BOOL __stdcall DATATBLS_GetAnimDataInfo(char* szPath, int* pOutLength, int* pOutAnimSpeed, int* pOutFirstFrameTagged)
{
	if (AnimDataRecord* pRecord = DATATBLS_GetAnimDataRecordFromPath(szPath))
	{
		...
		*pOutAnimSpeed = pRecord->dwAnimSpeed;
		...
		{
			if (pRecord->pFrameFlags[i] || i >= AnimDataRecord::MAX_FRAME_FLAGS)
			{
	...
	const AnimDataRecord& pDefaultRecord = DATATBLS_GetAnimData()->tDefaultRecord;
