...
The headless-container question is D2 game-side work, so it belongs with
D2Debugger and the conformance harness rather than with the Ghidra MCP server
or the fun-doc dashboard. It briefly sat in the ghidra-mcp repo purely because
that was the session that rescued it from `C:	mp`; it was moved here
2026-08-02 and removed from that repo's history.
